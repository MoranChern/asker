...
async def _spawn_gpu_worker(
    *,
    ...
    Returns:
      Concatenated 'answer' text emitted by the worker (used for agent decisions).

    Notes:
    - For decision steps we typically set forward_answer_events=False and forward_done_event=False,
      so the UI won't see tool-directive JSON or intermediate done signals.
    - Errors are always forwarded.
    """
    ...
    async with _GPU_SEM:
        ...
        try:
            ...
            while True:
                ...
                # Always collect answer for agent parsing
                if typ == "answer":
                    txt = obj.get("text") or ""
                    answer_buf.append(str(txt))

                # Forward events based on filters
                ...
                # If worker sends an error with traceback, surface traceback clearly
                if typ == "error":
                    seen_error = True
            ...
            # If worker crashed without emitting a structured error, synthesize one from stderr.
            ...
            # For final step, ensure client always receives done to unblock UI
...
async def ws_endpoint(websocket: WebSocket) -> None:
    ...
    try:
        while True:
            ...
            async def llm_call(messages: List[Dict[str, str]], forward_answer_events: bool, forward_done_event: bool) -> str:
            ...
            try:
                ...
                print("[server] agent exception:", e)
                print(tb)
                try:
                    await emit({"type": "error", "message": f"server exception: {e}", "trace": tb})
                    await emit({"type": "done"})
                except Exception:
                    pass
